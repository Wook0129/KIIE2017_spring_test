...
class Embedder:
    def __init__(self, *, inputs, target, num_instances, num_of_vars, total_num_of_bins,
                 num_of_bins_by_var, embedding_size):
        ...
        embed_instance = tf.nn.embedding_lookup(self.instance_embeddings,
                                                inputs[:, num_of_vars - 1])
        ...
        with tf.variable_scope('output_layer'):
            ...
            for i, num_of_bin_in_var in enumerate(num_of_bins_by_var):
                ...
                with tf.variable_scope('var_{}'.format(i)):
                    onehot_target = tf.squeeze(
                        tf.one_hot(indices=target, depth=num_of_bin_in_var), 1)
        ...
        self.optimizer = tf.train.GradientDescentOptimizer(config.learning_rate).minimize(
            self.loss)
...
def train_embedder(data, *, max_iteration=config.max_iteration,
    ...
    with tf.Session() as sess:
        inputs = tf.placeholder(tf.int32, shape=[None,
                                                 data_handler.num_of_vars])  # (num_vars - 1) + 1 instance_id
        ...
        for i in range(max_iteration):
            ...
            sess.run(embedder.optimizer,
                     feed_dict={inputs: train_inputs, target: train_labels})

            if (i + 1) % config.print_loss_every == 0:
                train_loss = sess.run(embedder.loss, feed_dict={inputs: train_inputs,
                                                                target: train_labels})
                print('Training Loss at step {} : {}'.format(i + 1, train_loss))

                val_inputs, val_labels = batch_generator.next_val_batch()
                val_loss = sess.run(embedder.loss,
                                    feed_dict={inputs: val_inputs, target: val_labels})
                print('Validation Loss at step {} : {}'.format(i + 1, val_loss))

                train_acc = embedder.accuracy.eval(
                    {inputs: train_inputs, target: train_labels})
                ...
                embedder.saver.save(sess, save_path=os.path.join(config.LOG_DIR,
                                                                 config.model_save_filename),
                                    global_step=(i + 1))
